...
import { X, PlayArrow as Play, Add as Plus, ThumbUp , Star, Event as Calendar, WatchLater as Clock, EmojiEvents as Award } from '@mui/icons-material';
          ...
          <div className="content-overlay absolute inset-0 flex items-end p-6">
            <div className="flex flex-col md:flex-row items-start md:items-end gap-6 w-full">
              ...
              <div className="movie-info flex-1 text-white">
                ...
                <p className="text-lg text-gray-300 mb-4">{movie.year} • {movie.rated} • {movie.runtime}</p>
                
                <div className="flex flex-wrap items-center gap-4 mb-6">
                  <div className="flex items-center space-x-1">
                    <Star className="w-5 h-5 text-yellow-400 fill-current" />
                    <span className="text-lg font-semibold">{movie.imdbRating}</span>
                    <span className="text-gray-300">({movie.imdbVotes.toLocaleString()} votes)</span>
                  </div>
                  {movie.metascore > 0 && (
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="moedal-sidbar space-y-6">
              <div>
                ...
                <p className="text-gray-300">{formatDirectors(movie.director)}</p>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {movie.genre.map((genre, index) => (
                    <span key={index} className="bg-zinc-700 px-2 py-1 rounded-full text-sm text-gray-300">
                      {genre}
                    </span>
                  ))}
                </div>
