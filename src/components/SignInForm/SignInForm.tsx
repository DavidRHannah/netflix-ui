...
export default function SignInForm() {
  ...
  const handleEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value && !validateEmail(value)) {
      setEmailError(t("signin.emailError"));
    } else {
      setEmailError("");
    }
  };
  ...
  return (
    <div className="bg-black/60 backdrop-blur-sm flex flex-col self-center gap-4 p-8 rounded-lg w-full max-w-md">
      ...
      <button
        ...
        className="w-full bg-red-600 hover:bg-red-700 hover:cursor-pointer text-white font-bold py-2 rounded transition-colors duration-200"
      >
        {t("signin.submit")}
      </button>
      ...
      <button
        type="button"
        className="w-full bg-gray-600/40 hover:bg-gray-600/50 hover:cursor-pointer text-white font-bold py-2 rounded transition-colors duration-200"
      >
        {t("signin.useCode")}
      </button>
